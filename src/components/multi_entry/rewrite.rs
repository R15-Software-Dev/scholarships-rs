...
use crate::components::{ActionButton, ValueType};
...
fn update_entry_list(list: &mut Vec<ValueType>, data: &HashMap<String, ValueType>) {
    if let Some(uuid) = get_uuid_from_map(&data) {
        if let Some(index) = list.iter()
            .position(|x| get_uuid_from_map(&x.as_map().unwrap().unwrap_or_default()) == Some(uuid.clone()))
        {
...
fn new_entry() -> ValueType {
    ...
    entry_map.insert("uuid".into(), ValueType::String(Some(Uuid::new_v4().to_string())));
...
pub fn MultiEntryRewrite(
    ...
    #[prop(optional)] schema: Vec<InputType>,
) -> impl IntoView {
    ...
    let data_reactive = RwSignal::new(data_map.get_untracked()
        .get(&data_member)
        .unwrap_or(&ValueType::List(None)).as_list()
...
fn Entry(
    ...
    #[prop(optional, into)] info_member: String,
) -> impl IntoView {
    view! {
